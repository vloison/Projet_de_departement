...
def main(args):
    ...
    selection_name = args.csv + triplet_to_str(config['first_date'])+'_'+triplet_to_str(config['last_date'])+'_'+str(nb_genres)
    ...
    if data_name[0].exists() and data_name[1].exists() and data_name[2].exists():
        ...
        posters, genres, ids = preprocess_data(Path(args.posters), selected_movies, nb_genres, config['image_size'], 
                                           verbose=args.verbose, logger=logger)
        if args.save:
            np.save(posters, data_name[0])
            np.save(genres, data_name[1])
            np.save(ids, data_name[2])
    ...
    if data_name[0].exists() and data_name[1].exists() and data_name[2].exists() and data_name[3].exists() and data_name[4].exists() and data_name[5].exists():
        ...
        train_posters, train_genres, train_ids, test_posters, test_genres, test_ids = split_data(
            ...
            method=config['split_method'], verbose=args.verbose, logger=logger)
        if args.save:
            np.save(train_posters, data_name[0])
            np.save(train_genres, data_name[1])
            np.save(train_ids, data_name[2])
            np.save(test_posters, data_name[3])
            np.save(test_genres, data_name[4])
            np.save(test_ids, data_name[5])
    ...
    if Path(model_name).exists():
        ...
        model = train_model(train_posters, train_genres, test_posters, test_genres, nb_epochs=config['nb_epochs'],
                        bacth_size = config['batch_size'], validation_split = config['validation_split'],
                        verbose=args.verbose, logger=logger)
        if args.save:
            model.save(Path(model_name))
    
    predicted_genres = model.predict(test_posters)
    multi_label(test_genres, predict_genres, verbose=verbose, logger=logger)
    return
